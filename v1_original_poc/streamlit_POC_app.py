...
st.title("Pipeline Health Monitor:heart: ")
...
def compute_pipeline_summary(history_df):
    ...
    for name in history_df["pipeline_name"].unique():
        ...
        successful_runs = len(pipe_df[pipe_df["status"] == "SUCCESS"])
        reliability_score = round((successful_runs / len(pipe_df)) * 100, 1)
        ...
        health_score = max(0, round(100 - risk_score, 1))
...
critical_pipelines = len(filtered_df[filtered_df["risk_level"] == "CRITICAL"])
high_risk_pipelines = len(filtered_df[filtered_df["risk_level"] == "HIGH RISK"])
watch_pipelines = len(filtered_df[filtered_df["risk_level"] == "WATCH"])
healthy_pipelines = len(filtered_df[filtered_df["risk_level"] == "HEALTHY"])
avg_health = round(filtered_df["health_score"].mean(), 1) if len(filtered_df) else 0
...
if len(top_pipeline) > 0:
    ...
    if top["risk_score"] >= 81:
        st.error(
            f"Immediate attention needed: **{top['pipeline_name']}** is the highest-risk pipeline "
            f"with a risk score of {top['risk_score']} and a health score of {top['health_score']}%."
        )
    elif top["risk_score"] >= 61:
        st.warning(
            f"Watch closely: **{top['pipeline_name']}** is trending high-risk "
            f"with a risk score of {top['risk_score']}."
        )
    elif top["risk_score"] >= 31:
        st.info(
            f"Monitor: **{top['pipeline_name']}** is the highest item in the current view, "
            f"but it is not critical."
        )
    else:
        st.success("No critical pipeline risk detected in the current view.")
    ...
    if len(action_items) > 0:
        ...
        for _, row in action_items.iterrows():
            st.write(
                ...
                f"{row['failures_7d']} failure(s) in 7d, trend: {row['trend']}, "
                f"last failure: {row['last_failure_ago']}."
            )
    else:
        st.markdown("**Recommended Actions:** None \u2014 all pipelines are operating within acceptable thresholds.")

filtered_history = history_df[history_df["service"].isin(selected_service)]
total_runs = len(filtered_history)
success_runs = len(filtered_history[filtered_history["status"] == "SUCCESS"])
success_rate = round(success_runs / total_runs * 100, 1) if total_runs > 0 else 0

col1, col2, col3, col4, col5, col6, col7 = st.columns(7)

col1.metric("Success Rate (7d)", f"{success_rate}%", border=True)
col2.metric("Avg Health", f"{avg_health}%", border=True)
col3.metric("Healthy", healthy_pipelines, border=True)
col4.metric("Watch", watch_pipelines, border=True)
col5.metric("High Risk", high_risk_pipelines, border=True)
col6.metric("Critical", critical_pipelines, border=True)
col7.metric("Failed Now", failed_pipelines, border=True)
...
with tab1:
    ...
    st.dataframe(
        filtered_df[
            ["pipeline_name", "service", "owner", "status", "criticality",
             "failures_24h", "warnings_24h", "failures_7d", "last_failure_ago",
             "trend", "runtime_spike", "risk_score", "health_score",
             "reliability_score", "risk_level", "last_error"]
...
with tab2:
    ...
    d1.metric("Risk Score", selected_row["risk_score"], border=True)
    d2.metric("Health Score", f"{selected_row['health_score']}%", border=True)
    d3.metric("Reliability", f"{selected_row['reliability_score']}%", border=True)
    d4.metric("Fails 24h", selected_row["failures_24h"], border=True)
    d5.metric("Fails 7d", selected_row["failures_7d"], border=True)
    ...
    st.write(f"**Last Failure:** {selected_row['last_failure_ago']}")
    st.write(f"**Runtime Change:** {selected_row['runtime_change_pct']}% compared to average")
    st.write(f"**Expected Frequency:** Every {selected_row['expected_frequency_hours']}h")
    st.write(f"**SLA Breach:** {'Yes' if selected_row['sla_breach'] else 'No'}")
    ...
    if len(common_errors) > 0:
        ...
        st.write("**Most Common Error:** No recent failures")

    st.subheader("Recent Runs")
    st.dataframe(
        selected_hist[["run_time", "status", "duration_seconds", "error_message"]].head(50),
        column_config={
...
with tab3:
    ...
    st.dataframe(
        filtered_df[
            ["pipeline_name", "service", "status", "last_run", "owner", "criticality",
             "latest_duration_seconds", "avg_duration_seconds", "risk_score",
             "health_score", "reliability_score", "trend", "risk_level"]
        ],
        column_config={
            "pipeline_name": st.column_config.TextColumn("Pipeline"),
            "service": st.column_config.TextColumn("Service"),
            "status": st.column_config.TextColumn("Status"),
            "last_run": st.column_config.DatetimeColumn("Last Run", format="MMM DD HH:mm"),
            "owner": st.column_config.TextColumn("Owner"),
            "criticality": st.column_config.TextColumn("Criticality"),
            "latest_duration_seconds": st.column_config.NumberColumn("Latest Runtime (s)"),
            "avg_duration_seconds": st.column_config.NumberColumn("Avg Runtime (s)"),
            "risk_score": st.column_config.ProgressColumn("Risk Score", min_value=0, max_value=100),
            "health_score": st.column_config.ProgressColumn("Health Score", min_value=0, max_value=100),
            "reliability_score": st.column_config.ProgressColumn("Reliability", min_value=0, max_value=100),
            "trend": st.column_config.TextColumn("Trend"),
    ...
    st.subheader("Pipeline Status Count")
    st.bar_chart(filtered_df["status"].value_counts())

with tab4:
    st.subheader("Recent Failure Details")
    ...
    if len(failure_history) == 0:
        ...
        st.dataframe(
            failure_history[
                ["pipeline_name", "service", "status", "run_time", "duration_seconds", "error_message"]
            ].head(100),
            column_config={
                "pipeline_name": st.column_config.TextColumn("Pipeline"),
                "service": st.column_config.TextColumn("Service"),
    ...
    st.subheader("Top Root Cause Trends")
    ...
    if len(root_cause_df) > 0:
        ...
        st.success("No failed runs available for root cause trend analysis.")
...
with tab5:
    ...
    st.dataframe(
        filtered_preds[
            ["pipeline_name", "service", "criticality", "recent_fail_rate",
             "historical_fail_rate", "trend", "trend_score", "signal"]
        ],
        column_config={
            "pipeline_name": st.column_config.TextColumn("Pipeline"),
            "service": st.column_config.TextColumn("Service"),
    ...
    st.subheader("Trend Score by Pipeline")
    st.bar_chart(filtered_preds.set_index("pipeline_name")["trend_score"])
...
with tab6:
    ...
    service_health = filtered_df.groupby("service").agg(
        ...
        failed_now=("status", lambda x: (x == "FAILED").sum()),
    ).reset_index()

    service_health["avg_health"] = service_health["avg_health"].round(1)
    service_health["avg_risk"] = service_health["avg_risk"].round(1)
    service_health["avg_reliability"] = service_health["avg_reliability"].round(1)

    st.dataframe(
        service_health,
        column_config={
            "service": st.column_config.TextColumn("Service"),
            "pipelines": st.column_config.NumberColumn("Pipelines"),
            "avg_health": st.column_config.ProgressColumn("Avg Health", min_value=0, max_value=100),
            ...
            "failed_now": st.column_config.NumberColumn("Failed Now"),
        },
        hide_index=True,
        use_container_width=True,
    )

    st.subheader("Average Health by Service")
    st.bar_chart(service_health.set_index("service")["avg_health"])

    st.subheader("Average Risk by Service")
    st.bar_chart(service_health.set_index("service")["avg_risk"])

with tab7:
    st.subheader("Stale Data Assets")
    st.caption(f"Showing pipelines with data older than {stale_threshold} hours")

    stale_df = filtered_df[filtered_df["stale_hours"] >= stale_threshold].sort_values(
        "stale_hours", ascending=False
    )

    sla_breaches = filtered_df[filtered_df["sla_breach"] == True]
    if len(sla_breaches) > 0:
        st.error(f"{len(sla_breaches)} pipeline(s) have exceeded their expected SLA window")

    if len(stale_df) == 0:
        st.success(f"No pipelines exceed the {stale_threshold}h staleness threshold!")
    else:
        st.dataframe(
            stale_df[
                ["pipeline_name", "service", "stale_hours", "expected_frequency_hours",
                 "sla_breach", "last_run", "status", "risk_level"]
            ],
            column_config={
                "pipeline_name": st.column_config.TextColumn("Pipeline"),
                "service": st.column_config.TextColumn("Service"),
                "stale_hours": st.column_config.NumberColumn("Stale (hrs)", format="%.1f"),
                "expected_frequency_hours": st.column_config.NumberColumn("Expected Freq (hrs)"),
...
st.caption(
    ...
    "failure trend analysis, risk scoring, stale data detection, operational prioritization, "
    "reliability scoring, and service-level health analysis."
)